...
- Reset selected clips back to a full-frame 1x1 placement while removing or neutralizing Grid Maker `Crop` / `Transform`
...
- `Reset` applies to selected video clips: it places the clip like a full-frame 1x1 Grid Maker cell and removes or neutralizes Grid Maker-managed `Crop` / `Transform` effects.
